...
class _MockStageState extends ConsumerState<_MockStage> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(clipBehavior: Clip.hardEdge, children: [
      ...
      // Panel matni: JAVOB bor → javob (raqamlar indigo); CHALA gap → "tanlang";
      // KIRISH (javob/taklif yo'q) → SALOMLASHUV (aiHello + aiIntro).
      Positioned(
        left: 80,
        top: 150,
        width: 548,
          ...
          child: v.answer.isNotEmpty
              ...
              : v.suggest
                  ...
                  : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(t['aiHello'] ?? 'Assalomu alaykum!',
                          style: const TextStyle(fontSize: 40, fontWeight: FontWeight.w800, color: _cInk)),
                      const SizedBox(height: 10),
                      Text(t['aiIntro'] ?? '',
                          style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w500, color: _cInk2, height: 1.4)),
                    ]),
        ),
      ),
